...
export const useReviewStore = defineStore('review', {
  ...
  getters: {
    getEventRating: (state) => (eventId) => {
      ...
      const average = count > 0
        ? eventReviews.reduce((sum, review) => sum + review.score, 0) / count
        : 0;
  ...
  actions: {
    addReview(eventId, userId, score, comment) {
      if (!this.reviews[eventId]) {
        this.reviews[eventId] = [];
      }

      const index = this.reviews[eventId].findIndex(
        review => review.userId === userId
      );

      if (index !== -1) {
        this.reviews[eventId][index] = { userId, score, comment };
      } else {
        this.reviews[eventId].push({ userId, score, comment });
      }
